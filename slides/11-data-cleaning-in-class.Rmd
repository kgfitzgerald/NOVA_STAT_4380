...
## Re-order income levels

.panelset[

.panel[.panel-name[Plot]
```{r rel-income2, out.width = "65%", echo = FALSE}
rel_inc_long <- rel_inc_long |>
  mutate(
    income = fct_relevel(
      income,
...
.panel[.panel-name[Code]
```{r ref.label = "rel-income2", fig.show = "hide"}
```
]
...
.panel[.panel-name[Plot]
```{r rel-income3, out.width = "65%", echo = FALSE}
ggplot(rel_inc_long, aes(y = religion, x = frequency, 
                         fill = fct_rev(income))) + 
  geom_col() 
```
...
.panel[.panel-name[Code]
```{r ref.label = "rel-income3", fig.show = "hide"}
```
]
